...
1. **Никогда не пиши код без MODULE_CONTRACT.** Контракт — источник истины. Код реализует контракт, а не наоборот.
2. **Никогда не меняй код без чтения GRACE-документов.** Порядок: `current-status.xml` → `knowledge-graph.xml` → `development-plan.xml` → `verification-plan.xml` → код.
3. **Никогда не коммить без passing verification.** Тесты должны пройти до коммита.
4. **Никогда не менять shared XML-артефакты без чтения их полностью.** `docs/*.xml` — load-bearing структура, не документация.
5. **После любого изменения кода — обновить MODULE_MAP, knowledge-graph, verification-plan (если тесты изменились).**
6. **Запрещено писать код напрямую.** Всегда: `$grace-plan` → `$grace-verification` → `$grace-execute` → `$grace-reviewer`.
7. **Если верификацию невозможно запустить — скажи точно почему.** Не пропускай, не имплицируй покрытие.
