...
#          creation, authenticated KPprotoN fake-TLS owner payload, idempotent assignment reuse, replay safety.
#   DEPENDS: M-041, M-042, M-043, M-044, M-045, M-040, M-004, M-005, M-020
#   LINKS: V-M-041, V-M-042, V-M-043, V-M-044, V-M-045, docs/plans/Phase-34.xml, docs/plans/Phase-41.xml
# END_MODULE_CONTRACT
#
# START_MODULE_MAP
...
#   LAST_CHANGE: v1.2.0 - Restored integration gate to Phase-41 KPprotoN fake-TLS ee owner payload.
...
async def test_verified_registration_issues_single_owner_mtproto_proxy(
    ...
    assert first_proxy["server"] == first_proxy["sni"]
    assert first_proxy["port"] == 443
    assert first_proxy["secret"].startswith("ee")
    assert first_proxy["credential_mode"] == "derived_per_sni"
