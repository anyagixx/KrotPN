...
//   PURPOSE: Static Phase-25 session-security gate for auth storage, refresh, logout, cookies, and blacklist expectations.
//   SCOPE: Reads governed auth files and fails when required Phase-25 guardrails drift.
...
function run() {
  ...
  requireText('frontend/src/pages/Register.tsx', "localStorage.setItem('refresh_token'", 'user register stores refresh token for current rollback-compatible flow')
